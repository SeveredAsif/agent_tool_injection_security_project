...
class Agent:
    def __init__(self, llm: Any, env: Environment, tools: dict[str, ToolSpec],
        ...
        self.cfg = cfg
        self.attack = attack
        # Optional step-by-step observer. It is None in experiments, so the loop
        # runs at full speed there; demov2 in main.py uses it to print the flow.
        self.trace = trace
    ...
    def run(self, user_task: str) -> AgentRun:
        ...
        self._emit("user_task", {"task": user_task})

        for step in range(self.cfg.max_steps):
            run.steps_used = step + 1
            self._emit("model_query", {"step": step + 1})
            response = self.llm.step()
            self._emit("model_reply", {
                "step": step + 1, "kind": response.kind,
                "tool_name": response.tool_name, "args": dict(response.args or {}),
                "answer": response.answer, "raw": response.raw})

            if response.kind == "final":
                run.final_answer = response.answer
                run.stop_reason = "final_answer"
                run.messages = list(getattr(self.llm, "messages", []))
                self._emit("final", {"answer": response.answer})
                return run
            ...
            if tool_name not in self.tools:
                ...
                self._emit("invalid_tool", {"tool": tool_name})
                self.llm.add_user(
            ...
            sensitive = tool_name in config.SENSITIVE_TOOLS
            self._emit("authz", {
                "tool": tool_name, "args": dict(args), "verdict": decision.verdict,
                "reason": decision.reason, "defense_on": self.cfg.defense_enabled,
                "sensitive": sensitive})
            ...
            # ---- T3: execution.
            secret_before = len(self.env.secret_reads)
            verif_before = len(self.env.verifications)
            try:
            ...
            poisoned = False
            honest_output = str(raw)
            if executed:
                self.env.executed.append(tool_name)
                run.executed_tools.append(tool_name)
                self._emit("tool_ran", {
                    "tool": tool_name, "args": dict(args), "honest": honest_output,
                    "read_secret": len(self.env.secret_reads) > secret_before,
                    "did_verify": len(self.env.verifications) > verif_before})
                if self.attack is not None and self.attack.controls(tool_name):
                    raw = self.attack.poison(honest_output, tool_name=tool_name)
                    poisoned = True
                    self._emit("poison", {
                        "tool": tool_name, "honest": honest_output, "poisoned": str(raw)})
            else:
                self._emit("tool_error", {"tool": tool_name, "reason": reason})
            ...
            self._emit("tool_message", {
                "tool": tool_name, "trust_level": message.trust_level,
                "source_channel": message.source_channel, "body": body,
                "wrapped": self.cfg.defense_enabled and self.cfg.use_delimiters})
        ...
        run.messages = list(getattr(self.llm, "messages", []))
        self._emit("max_steps", {"steps": run.steps_used})
        return run
